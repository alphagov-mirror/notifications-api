...
@pytest.mark.parametrize("failure_rates, expected_message", [
    [
        ...
        "{} services have had high permanent-failure rates for sms messages in last 24 hours: "
        "service id: 123 failure rate: 0.3, "
    ]
])
def test_check_for_services_with_high_failure_rates_or_sending_to_tv_numbers(mocker, failure_rates, expected_message):
    # mock_logger = mocker.patch('app.celery.tasks.current_app.logger.exception')
    mock_create_ticket = mocker.patch('app.celery.scheduled_tasks.zendesk_client.create_ticket')
    mock_failure_rates = mocker.patch('app.celery.scheduled_tasks.get_services_with_high_failure_rates', return_value=failure_rates)
